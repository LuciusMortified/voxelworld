...
auto world_grid::set_voxel(
    ...
    // Воксель на оболочке чанка — половина шва: сосед был смешен под старую
    // плоскость и обязан узнать о новой.
...
// Небесный свет доносится на пятнадцать шагов от места изменения, поэтому правка в
// пределах пятнадцати вокселей от бока колонки меняет свет и в соседней — причём
// через угол так же охотно, как через сторону, отчего названа и диагональ.
//
...
auto world_grid::refresh_chunk(
    ...
    // Погребённую породу стоит рисовать ровно с того момента, как в неё вкопались.
    if (c.ensure_entity()) {
        ++drawn_chunks_;
    }
