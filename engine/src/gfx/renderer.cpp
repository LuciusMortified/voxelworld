...
#include "vw/gfx/mesh.h"
#include "vw/gfx/shader.h"
...
renderer::renderer(vulkan_context& context, window& window)
    ...
    fragment_shader_ = std::make_unique<shader>(
        *context_,
        "shaders/voxel.frag.spv",
        shader_type::FRAGMENT
    );

    debug_vertex_shader_ = std::make_unique<shader>(
        *context_,
        "shaders/debug.vert.spv",
        shader_type::VERTEX
    );
    debug_fragment_shader_ = std::make_unique<shader>(
        *context_,
    ...
    debug_vertex_buffer_ =
        std::make_unique<vertex_buffer>(*context_, initial_size);
...
void renderer::end_frame() {
    ...
    VkResult result =
        vkQueuePresentKHR(context_->get_present_queue(), &present_info);
...
void renderer::render(world& world, camera& camera) {
    ...
    render_debug_primitives(camera);
    ...
    if (vkEndCommandBuffer(command_buffers_[current_image_index_]) !=
        VK_SUCCESS) {
...
void renderer::render_world(const world& world, const camera& camera) const {
    ...
    const VkPipeline current_pipeline =
...
void renderer::render_debug_primitives(const camera& camera) {
    ...
    const VkBuffer vertex_buffer  = debug_vertex_buffer_->get_buffer();
...
void renderer::update_debug_vertex_buffer() {
    ...
    const VkDeviceSize required_size =
        sizeof(debug_vertex) * debug_vertices.size();
    if (required_size > debug_vertex_buffer_->get_size()) {
        debug_vertex_buffer_ =
            std::make_unique<vertex_buffer>(*context_, required_size);
    }
...
void renderer::create_render_pass() {
    ...
    dependency_imgui.srcStageMask =
        VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
    dependency_imgui.srcAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
    dependency_imgui.dstStageMask =
        VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
    dependency_imgui.dstAccessMask = VK_ACCESS_COLOR_ATTACHMENT_READ_BIT |
...
void renderer::create_graphics_pipeline() {
    ...
    rasterizer.frontFace =
        VK_FRONT_FACE_CLOCKWISE;  // Изменено для совместимости с
                                  // инвертированной Y
...
void renderer::create_uniform_buffers() {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        uniform_buffers_[i] =
            std::make_unique<uniform_buffer>(*context_, buffer_size);
    }
}
...
void renderer::init_imgui() {
    ...
    init_info.QueueFamily =
        context_->get_queue_families().graphics_family.value();
...
void renderer::cleanup_descriptor_set_layout() {
...
void renderer::create_image(
    ...
    if (vkCreateImage(context_->get_device(), &image_info, nullptr, &image) !=
        VK_SUCCESS) {
