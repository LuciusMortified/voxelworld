...
void mesh::draw_indexed(VkCommandBuffer command_buffer) {
    if (index_count_ > 0) {
        vkCmdDrawIndexed(
            command_buffer,
            static_cast<uint32>(index_count_),
            1,
            0,
            0,
            0
...
mesh_data simple_mesh_generator::generate_mesh_data(
    const std::shared_ptr<model>& model
) {
    if (!model) {
        return mesh_data{};
        ...
        for (int y = 0; y < model->height(); y++) {
            for (int z = 0; z < model->depth(); z++) {
                if (voxel voxel_obj = model->get_voxel(x, y, z);
                    !voxel_obj.is_empty(
                    )) {  // Если воксел существует (не прозрачный)
...
mesh_data greedy_mesh_generator::generate_mesh_data(
    const std::shared_ptr<model>& model
) {
    if (!model) {
        throw std::runtime_error("model is null");
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                if (!visited[x][y] && mask[x][y] != colors::empty) {
                    ...
                    while (x + w < width && mask[x + w][y] == color &&
                           !visited[x + w][y]) {
                        w++;
                    }
