...
engine::engine(int width, int height, std::string_view title) {
    ...
    debug_tool_ = std::make_unique<debug_tool>(*renderer_);

    // Default empty app to avoid null checks
    app_ = std::make_unique<app>();

    window_resize_sub_ = window_->on<events::window_resize>(
    ...
    key_press_sub_ = window_->on<events::key_press>(
        [this](events::key_press& event) -> bool {
            if (event.key == input::key::F12 && event.with(input::mod::CTRL)) {
