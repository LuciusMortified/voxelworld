...
debug_window::debug_window(renderer& renderer)
    : renderer_(&renderer), last_fps_update_(std::chrono::high_resolution_clock::now()) {
...
void debug_window::update_fps_history(float fps) {
    ...
    if (!fps_history_.empty()) {
        average_fps_ = std::accumulate(fps_history_.begin(), fps_history_.end(), 0.0f) /
...
void debug_window::render_fps_window() {
    ...
    if (ImGui::Begin("Debug Tool", &visible_, window_flags)) {
        ...
        ImGui::Text("Avg: %.1f Min: %.1f Max: %.1f", average_fps_, min_fps_, max_fps_);
...
void debug_window::render_fps_graph() const {
    if (fps_history_.empty()) {
        return;
    }
