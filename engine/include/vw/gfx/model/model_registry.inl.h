...
[[nodiscard]] inline auto model_registry::create(std::string_view name, int width, int height, int depth) -> std::shared_ptr<model> {
    auto new_model = std::make_shared<model>(identity_pool_, width, height, depth);
    models_[std::string(name)] = new_model;
    return new_model;
}

[[nodiscard]] inline auto model_registry::create_unnamed(int width, int height, int depth) -> std::shared_ptr<model> {
    return std::make_shared<model>(identity_pool_, width, height, depth);
}

[[nodiscard]] inline auto model_registry::create_clone(std::string_view name) -> std::shared_ptr<model> {
    auto original = get(name);
    if (!original) {
        return nullptr;
    }
    auto cloned_model = std::make_shared<model>(identity_pool_, original->width(), original->height(), original->depth());
