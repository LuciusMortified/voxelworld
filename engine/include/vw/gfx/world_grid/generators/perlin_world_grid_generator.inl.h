...
#ifndef VW_GFX_WORLD_GRID_GENERATORS_PERLIN_WORLD_GRID_GENERATOR_INL_H
#define VW_GFX_WORLD_GRID_GENERATORS_PERLIN_WORLD_GRID_GENERATOR_INL_H
...
inline perlin_world_grid_generator::perlin_world_grid_generator(params p) : params_(p) {
...
inline auto perlin_world_grid_generator::fade(float64 t) -> float64 {
    return t * t * t * (t * (t * 6.0 - 15.0) + 10.0);
}

inline auto perlin_world_grid_generator::lerp(float64 t, float64 a, float64 b) -> float64 {
    return a + t * (b - a);
}

inline auto perlin_world_grid_generator::grad(int32 hash, float64 x, float64 y) -> float64 {
    int32 h = hash & 3;
    float64 u = h < 2 ? x : y;
    float64 v = h < 2 ? y : x;
...
inline auto perlin_world_grid_generator::noise2d(float64 x, float64 y) const -> float64 {
...
inline auto perlin_world_grid_generator::octave_noise(float64 x, float64 y) const -> float64 {
    float64 total = 0.0;
    float64 freq = 1.0;
    float64 amp = 1.0;
    float64 max_amp = 0.0;
...
inline auto perlin_world_grid_generator::ridged_noise(float64 x, float64 y) const -> float64 {
    float64 total = 0.0;
    float64 freq = 1.0;
    float64 amp = 1.0;
    float64 max_amp = 0.0;
...
inline auto perlin_world_grid_generator::continent_at(float64 nx, float64 nz) const -> float64 {
    float64 c = octave_noise(nx * params_.continent_frequency,
                             nz * params_.continent_frequency);
    return (c + 1.0) * 0.5;
}

inline auto perlin_world_grid_generator::height_at(int32 wx, int32 wz) const -> int32 {
    auto nx = static_cast<float64>(wx);
    auto nz = static_cast<float64>(wz);

    float64 warp_x = octave_noise(nx * params_.warp_frequency + 5.2,
                                  nz * params_.warp_frequency + 1.3) * params_.warp_strength;
    float64 warp_z = octave_noise(nx * params_.warp_frequency + 9.7,
                                  nz * params_.warp_frequency + 6.1) * params_.warp_strength;
    ...
    float64 terrain = octave_noise(warped_x * params_.terrain_frequency,
                                   warped_z * params_.terrain_frequency);
    terrain = (terrain + 1.0) * 0.5;

    float64 ridge = ridged_noise(warped_x * params_.ridge_frequency,
                                 warped_z * params_.ridge_frequency);
...
inline auto perlin_world_grid_generator::color_at(
    ...
    if (continent >= 0.7) {
        if (depth == 0) {
            if (surface_y > params_.mountains_height + 20) return colors::white;
            return colors::stone;
        }
        return colors::stone;
    }
    ...
    if (depth == 0) {
        if (surface_y > params_.mountains_height + 10) return colors::white;
        if (surface_y > params_.hills_height + 10) return colors::stone;
        return colors::grass;
    }
    if (depth < 3) return colors::dirt;
    return colors::stone;
}
...
inline auto perlin_world_grid_generator::get_chunk_y_range(
    ...
    int32 samples[] = {
        height_at(wx0, wz0), height_at(wx1, wz0),
        height_at(wx0, wz1), height_at(wx1, wz1),
        height_at(wxm, wzm),
    };
...
inline auto perlin_world_grid_generator::generate_chunk(vec3i coord) -> chunk_data {
    constexpr int32 s = chunk<>::size;
...
#endif  // VW_GFX_WORLD_GRID_GENERATORS_PERLIN_WORLD_GRID_GENERATOR_INL_H
