...
#ifndef VW_GFX_WORLD_SYSTEMS_ANIMATION_STATE_MACHINE_SYSTEM_INL_H
#define VW_GFX_WORLD_SYSTEMS_ANIMATION_STATE_MACHINE_SYSTEM_INL_H
...
animation_state_machine_system<WC>::animation_state_machine_system(
    context_type& context, animation_system_type& anim_system
)
    : context_(&context), anim_system_(&anim_system) {}

template <typename WC>
void animation_state_machine_system<WC>::update() {
    auto view =
        context_->registry()
            .template view<animation_state_machine_component, animation_player_component>();
...
template <typename WC>
animation_state_machine_system<WC>::modifier::modifier(
    animation_state_machine_component* component
)
    : component_(component) {}

template <typename WC>
void animation_state_machine_system<WC>::modifier::add_machine(
    size_t index, animation_state_machine machine
) const {
...
template <typename WC>
void animation_state_machine_system<WC>::modifier::fire_trigger(
...
template <typename WC>
auto animation_state_machine_system<WC>::modify(
    entity ent
) -> modifier {
    auto& comp = context_->registry().template get<animation_state_machine_component>(ent);
    return modifier(&comp);
}

}  // namespace vw::gfx

#endif  // VW_GFX_WORLD_SYSTEMS_ANIMATION_STATE_MACHINE_SYSTEM_INL_H
