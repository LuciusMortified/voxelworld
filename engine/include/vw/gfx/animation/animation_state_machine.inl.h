...
#ifndef VW_GFX_ANIMATION_STATE_MACHINE_INL_H
#define VW_GFX_ANIMATION_STATE_MACHINE_INL_H

#include <algorithm>

namespace vw::gfx {

inline void animation_state_machine::add_state(state_node state) {
    auto name = state.name;
    states_.emplace(std::move(name), std::move(state));
}

inline void animation_state_machine::set_entry_state(std::string_view name) {
    entry_state_   = std::string(name);
    current_state_ = entry_state_;
}

inline auto animation_state_machine::get_current_state() const -> const std::string& {
    return current_state_;
}

inline auto animation_state_machine::get_current_state_node() const -> const state_node* {
    auto it = states_.find(current_state_);
    if (it == states_.end()) return nullptr;
    return &it->second;
}

inline auto animation_state_machine::evaluate(const animation_layer& layer, trigger_set& triggers)
...
inline void animation_state_machine::apply_transition(const transition_result& result) {
    current_state_ = result.target_state;
}

}  // namespace vw::gfx

#endif  // VW_GFX_ANIMATION_STATE_MACHINE_INL_H
