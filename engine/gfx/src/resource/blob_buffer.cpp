...
auto blob_buffer::update(
    ...
    for (const auto& [ent, blob, transform] : view) {
        ...
        // Колонка, в которой живёт это пятно: от середины тела вниз до конца
        // досягаемости, не шире, чем диск в самом широком месте. Верх — середина,
        // а не макушка: выше неё тело видно из-под земли уже не сверху, и вклад
        // там ровно нулевой (см. blobShadow), так что полтела высоты — это не
        // запас, а то, чего в списках быть не должно.
        //
        // Диск сужается по пути вниз, а капсула нет, поэтому внизу посадка
        // свободная — платой за это становятся напрасно перечисленные кластеры,
        // каждый из которых стоит пикселю земли одного неудачного сравнения.
        const float32 top    = pos.y + (height * 0.5F);
