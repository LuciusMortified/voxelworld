...
auto blob_buffer::update(
    ...
    for (const auto& [ent, blob, transform] : view) {
        ...
        //
        // Границы есть только у сущности, которую отслеживает пространственная
        // система, а она отслеживает несущих модель. Корень иерархии, сам ничего не
        // держащий, хранит коробку нулевого размера, и для таких есть только
        // трансформ.
