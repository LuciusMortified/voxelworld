...
- NEVER добавляй `import std` в **интерфейсные** партиции до M6: пока
  потребители включают стандартные заголовки текстуально, это ломает их сборку.
  В имплементационных юнитах (`.cpp`) `import std` обязателен — они никому не
  видны, и это быстрее GMF
...
- Стандартная библиотека: в `.cppm` — через global module fragment (`module;` +
  `#include`), и тот же набор заголовков должен быть в
  `vw/core/detail/module_prelude.h`; в `.cpp` — `import std;`
