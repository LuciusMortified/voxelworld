...
|------|-----------|------|-------|
| M0 | Тулчейн-спайк: CMake 4.x, Ninja, `import std`, `import vulkan`, матрица MSVC+Clang | низкий | зелёная сборка скелета на обоих компиляторах |
...
| D2 | Vulkan-биндинги | Vulkan-Hpp через официальный модуль `import vulkan` (Khronos) |
...
| D10 | Макросы конфигурации движка | Один традиционный заголовок `vw/config.h` (асерты, уровни логов); всё остальное — import |
...
4. Прекомпиляция модуля `vulkan` из SDK и smoke-тест `import vulkan;` с созданием `vk::ApplicationInfo`.
...
- Замерены и записаны: время полной сборки скелета, время инкрементальной сборки после правки интерфейса и после правки имплементации (базлайн для всех дальнейших фаз).
- Задокументирован выбор Clang-конфигурации (clang-cl vs clang+libc++).
...
Имплементация (`log.cpp`): форматирование в вызывающем потоке (уже сделано шаблоном выше), MPSC-очередь строк, фоновый поток с консольным (`std::print` + цвет по level) и файловым синком, flush по таймеру и на `error`. ~150–200 строк. Compile-time отсечение уровней — через `vw/config.h` (D10): `if constexpr (static_cast<int>(lvl) >= VW_LOG_MIN_LEVEL)`.

Зависимость spdlog удаляется из vcpkg.json по завершении фазы.

## Приёмка M1

- `vw_core` собирается как модуль на обоих компиляторах; остальной движок (ещё header-only) потребляет его смешанным режимом: TU приложений добавляют `import vw.core;` рядом с существующими include (легальный переходный режим — старые заголовки core остаются тонкими шимами `import vw.core;` + `using`, пока живы потребители).
...
- Ни один заголовок движка не содержит параметра `WC`; `grep -r "typename WC\|class WC"` пуст.
- Все тесты ECS и мира зелёные; поведение lifecycle (create/destroy/generation) бит-в-бит с текущим.
- Бенчмарк системной итерации: деградация ≤ 10% (ожидаемо ~0).
...
5. **CLAUDE.md переписывается** — половина текущих конвенций описывает header-only идиомы (`.inl.h`-пары, include-порядок). Новые конвенции: структура модуля (интерфейсная партиция ↔ имплементационные юниты), правила экспорта, правило D10 про единственный config.h, правило интерфейсной гигиены vk::, шаблоны «только на поверхности API».
6. docs/ENGINE.md обновляется под новую структуру каталогов.

## Приёмка M6

- В репозитории нет `.inl.h`-файлов; `#include` наших собственных заголовков остаётся только для `vw/config.h`.
